...
const DATA_TIMEOUT_MS = 1500;   // per-pull capture timeout (parked senders go quiet)
...
class PreviewManager {
	...
	async attach(p) {
		...
		p.starting = false;
		p.state = 'live';
		this.log(`[prev] ${p.player}: receiving ${source.name}`);
		this.pullLoop(p);
	}
	...
	async pullLoop(p) {
		while (p.active && p.receiver) {
			const t0 = Date.now();
			try {
				const f = await p.receiver.data(DATA_TIMEOUT_MS);
			...
			} catch { /* timeout (quiet/parked sender) or transient error — keep pulling */ }
			const spend = Date.now() - t0;
			if (p.active) await sleep(Math.max(0, PULL_INTERVAL_MS - spend));
		}
		// in-flight capture has settled — now the receiver may be released to GC
		p.receiver = null;
		p.done = true;
		this.gcSoon();
	}
